> ⚠️ SPORTMIND STALENESS: Last verified 2026-09-14 · Tier A · 30-day cycle · Status: CURRENT · Next check due: 2026-10-14 · Apply with normal confidence. Verify individual jurisdiction files for current status before applying to live analysis. Load jurisdiction-specific files for granular detail.

# Global Cryptoasset Regulatory Landscape

**Enduring framework. Jurisdiction tier map and multi-gate signal logic.**
**Version:** v4.6.70 · Last verified: 2026-09-14

> Library Rule: Tier classifications and structural approaches only.
> Specific application deadlines, current enforcement actions, and
> firm-specific compliance status are perishable — SMI briefings only.

---

## Four-tier jurisdiction classification

```
TIER 1 — ACTIVE COMPREHENSIVE FRAMEWORK (binary gate applies):

  European Union
    Framework: MiCA (Markets in Crypto-Assets Regulation)
    Administrator: ESMA + national competent authorities
    Fan token classification: utility tokens, in scope
    Key mechanic: authorisation required for trading platforms
    See: macro/regulatory/eu-mica.md

  United Kingdom
    Framework: FSMA (Cryptoassets) Regulations 2026
    Administrator: Financial Conduct Authority (FCA)
    Fan token classification: qualifying cryptoassets, A&D regime applies
    Key mechanic: binary authorisation gate, white paper disclosure
    See: macro/regulatory/uk-cryptoasset-regime.md

  Japan
    Framework: Payment Services Act (PSA) + Financial Instruments and
    Exchange Act (FIEA) — most mature APAC framework
    Administrator: Financial Services Agency (FSA)
    Fan token classification: cryptoasset exchange services regulated;
    FIEA amendment (2026) moves toward financial instrument classification
    Key mechanic: licensed exchange required, strict AML/CFT obligations
    Socios relevance: Japan is a significant Socios user market

    JAPAN — FIEA AMENDMENT (July 2026):
    Status: ENACTED — House of Councillors passed July 15 2026
    Framework: Financial Instruments and Exchange Act (FIEA) amendment
    Regulator: Financial Services Agency (FSA)

    Key structural changes:
    · Crypto assets reclassified from payment instruments (Payment Services
      Act) to financial products under FIEA — same category as stocks and bonds
    · 102 cryptocurrencies now classified as financial instruments
    · Insider trading restrictions now apply to crypto markets
    · Annual disclosure obligations for token issuers
    · Retail investment cap: 2 million JPY limit on high-risk tokens
    · Penalties for illegal crypto business: up to 10 years imprisonment

    Timelines:
    · FIEA framework takes effect: fiscal 2027
    · Spot crypto ETFs: Tokyo Stock Exchange pathway open, launch targeted 2027-2028
    · Capital gains tax reduction: 55% → 20% flat rate, effective January 2028
    · Stablecoins: explicitly excluded from FIEA scope

    Fan token implications:
    · No explicit fan token classification in the law
    · Fan tokens fall under broader crypto-asset definition unless excluded
    · Insider trading restrictions apply — undisclosed match outcomes or
      partnership decisions could constitute material non-public information
    · Japan FSA has 14 million+ active crypto accounts — significant retail base
    · Regulatory framework takes effect 2027 — current operations unaffected

    Agent rule: Japan is now a securities-law jurisdiction for crypto.
    Fan tokens operating there face the same disclosure and insider-trading
    obligations as listed securities from fiscal 2027 onward.

  Singapore
    Framework: Payment Services Act (PSA) live Jan 2020 · PSA Amendment
    (PS(A)A) live Apr 2024 · DTSP regime live 30 Jun 2025
    Administrator: Monetary Authority of Singapore (MAS)
    Fan token classification: probable DPT (Digital Payment Token) ·
    substance-over-form · no confirmed MAS guidance as of 2026-09-14
    Key mechanic: MPI/SPI licence required for platforms · DTSP licence
    for issuers/distributors · no CGT · GST exempt on DPTs
    Friction: MODERATE · reduces to LOW on confirmed MAS licence
    Socios/Chiliz: MAS licence UNCONFIRMED as of 2026-09-14
    See: macro/regulatory/singapore.md

TIER 2 — PARTIAL OR DEVELOPING FRAMEWORK (monitor, gate developing):

  Hong Kong
    Framework: VATP (Virtual Asset Trading Platform) mandatory licensing
    live June 2023 · retail access explicitly PERMITTED on licensed platforms
    Administrator: Securities and Futures Commission (SFC)
    Fan token classification: probable non-security virtual asset ·
    substance-over-form · no confirmed SFC guidance as of 2026-09-14
    Key mechanic: SFC VATP licence required · retail access permitted ·
    SFC perimeter actively expanding · no CGT · no GST/VAT
    Friction: LOW-MODERATE · most permissive major Asian jurisdiction ·
    reduces to LOW on confirmed SFC licence
    Socios/Chiliz: SFC VATP licence UNCONFIRMED as of 2026-09-14
    Note: Mainland China ≠ Hong Kong — do not conflate
    See: macro/regulatory/hong-kong.md

  South Korea
    Framework: VAUPA (Virtual Asset User Protection Act) LIVE July 2024 ·
    DABA (Digital Asset Basic Act) PENDING H2 2026
    Administrator: FSC (Financial Services Commission) · FSS · KoFIU
    Fan token classification: OPEN — no FSC guidance confirmed ·
    probable virtual assets under VAUPA · substance-over-form
    Key mechanic: mandatory VASP registration · no confirmed fan token
    exemption · CGT 20% on gains >KRW 2.5M annual exemption (verify
    implementation at nts.go.kr)
    Friction: HIGH · unconfirmed VASP registration + no fan token
    exemption · DABA passage = BINARY EVENT · escalate immediately
    Socios/Chiliz: VASP registration UNCONFIRMED as of 2026-09-14
    See: macro/regulatory/south-korea.md

  Brazil
    Framework: Banco Central do Brasil (BCB) VASP authorisation regime ·
    BCB authorisation deadline: October 30 2026
    Fan token classification: virtual assets, VASP licensing required
    Key mechanic: AML/CFT + capital requirements · progressive CGT
    15%–22.5% with R$35,000 monthly exemption (MP 1.303/2025 LAPSED —
    never apply 17.5% flat rate)
    Friction: MODERATE
    Socios/Chiliz BCB authorisation: UNCONFIRMED as of 2026-09-14
    See: macro/regulatory/brazil.md · intelligence/country-scan/brazil.md

  Australia
    Framework: Capital gains tax framework exists; no comprehensive
    crypto regime yet — consultation ongoing
    Status: regulatory framework expected to develop 2026-2027
    Monitor: ASIC for any fan token-specific developments

TIER 3 — RESTRICTIVE OR MINIMAL (demand signal materially impaired):

  Russia
    Framework: 282-FZ (Federal Law on Digital Currency) · STATUTORY
    SUPPRESSOR CONFIRMED · live September 1 2026
    Fan token classification: STRUCTURALLY EXCLUDED IN STATUTE ·
    approved asset list: BTC/ETH/USDT only · fan tokens NOT eligible
    Key mechanic: statutory exclusion — not regulatory ambiguity ·
    no near-term recovery path unless statute amended
    EU sanctions: Socios/Chiliz (MFSA Malta) cannot serve Russian users
    Signal: MAXIMUM SUPPRESSION — bidirectional access restrictions
    See: macro/regulatory/russia.md · intelligence/country-scan/russia.md
    AGENT RULE: NEVER state fan tokens are eligible in Russia.
    282-FZ is a STATUTORY SUPPRESSOR — not a gap or ambiguity.

  India
    Framework: Partial oversight — exchange registration required
    but no broad regulatory regime
    Tax: 30% on crypto gains — suppresses retail demand materially
    Fan token signal: impaired by tax burden and regulatory uncertainty

  China
    Framework: Ban on most cryptoasset activity since 2021
    Exception: Hong Kong operates as distinct jurisdiction (see Tier 1/2)
    Fan token signal: mainland China demand effectively zero
    AGENT RULE: Never conflate Mainland China (BANNED) with Hong Kong
    (PERMITTED under VATP). One Country Two Systems — separate regimes.

TIER 4 — NOT YET IN SCOPE or DEVELOPING (no active framework, low Socios presence):

  South Africa
    Framework: FSCA primary regulator · CARF live 2 March 2026 ·
    first African nation to implement OECD CARF ·
    ITR12 4522 enforcement active · HP-9 ACTIVE
    CGT: 18% maximum effective rate · R40,000 annual exclusion
    Fan token signal: LOW-MEDIUM negative modifier from SARS enforcement
    See: macro/regulatory/south-africa-sars.md

  Pakistan
    Framework: PVARA (Pakistan Virtual Assets Regulatory Authority) ·
    Virtual Assets Act 2026 · September 2026 VASP registration
    Fan token classification: OPEN · Path C (ambiguous) outcome
    Socios/Chiliz NOC: UNCONFIRMED
    Signal: binary gate UNCONFIRMED · monitor pvara.gov.pk

  Most of Africa (ex-South Africa), Southeast Asia (ex-Singapore),
  Central Asia — no active fan token framework. Monitor via SMI
  for Socios partnerships or new regulatory frameworks.
```

---

## Key global regulatory themes (as of 2026-09-14)

```
THEME 1 — MiCA FULL APPLICATION:
  EU27 fully in force 30 December 2024. CASP authorisation wave
  ongoing. Socios/Chiliz MFSA Malta authorised September 2025 —
  EU27 passporting operative. Transitional period ended June 30 2026.
  Enforcement regime stable. Individual fan token white paper
  registrations in progress via MFSA notification to ESMA.
  See: macro/regulatory/eu.md · macro/regulatory/eu-mica.md

THEME 2 — CARF EXPANSION:
  OECD Crypto-Asset Reporting Framework adoption accelerating.
  South Africa: first African adopter · live 2 March 2026.
  EU DAC8: active. More jurisdictions expected.
  Impact: offshore crypto holdings no longer sheltered from tax
  authority visibility in CARF-adopting jurisdictions.

THEME 3 — VASP LICENSING WAVE:
  Multiple jurisdictions activated or tightened VASP licensing
  in 2024-2026:
    · South Korea: VAUPA live July 2024
    · Singapore: PSA Amendment Apr 2024 · DTSP live Jun 2025
    · Hong Kong: VATP mandatory Jun 2023 · perimeter expanding
    · Pakistan: PVARA · Virtual Assets Act 2026
  Common pattern: mandatory licensing + AML/CFT + investor protection.

THEME 4 — UK REGIME MATERIALISING:
  FCA authorisation gateway opens 30 September 2026.
  Full regime go-live 25 October 2027.
  HP-10 ELEVATED: NCA freeze extended ~January 2027 ($SFA affected).
  PTG eligibility for $SFA/$BELG: UNRESOLVED.
  See: macro/regulatory/uk-cryptoasset-regime.md

THEME 5 — EQUITY TOKEN CATEGORY EMERGING:
  Socios Equity Token announced 27 August 2026 · Securitize
  infrastructure partner confirmed 2 September 2026.
  Distinct from fan tokens — probable regulated security in most
  jurisdictions. Fan Tokens ≠ Equity Tokens — never conflate.
  File: equity-token/socios-equity-token.md (HOLD · Conditions
  2+3 unresolved · no named club · no tokenomics published).

THEME 6 — STATUTORY SUPPRESSORS:
  Russia (282-FZ): CONFIRMED STATUTORY SUPPRESSOR. Fan tokens
  structurally excluded in statute. Approved list: BTC/ETH/USDT.
  No near-term recovery path unless statute amended.
  Model as permanent suppressor — not regulatory ambiguity.
```

---

## Friction tier summary (as of 2026-09-14)

```
LOW friction (tax-advantaged individual holders):
  Portugal    — 0% CGT after 365 days holding
  Germany     — 0% CGT after 12 months holding
  Belgium     — 0% CGT (Type A private investor)
  Hong Kong   — 0% CGT · 0% GST/VAT · retail access permitted
  Singapore   — 0% CGT · GST exempt on DPTs

MODERATE friction:
  Singapore   — MODERATE (zero tax offset by unconfirmed licence)
  UK          — LOW-MODERATE → elevated to MODERATE by HP-10/NCA
  Brazil      — MODERATE (progressive CGT · BCB deadline Oct 2026)
  UAE         — LOW-MODERATE (VARA/ADGM · verify at jurisdiction file)

MODERATE-HIGH friction:
  France      — 30% PFU (Prélèvement Forfaitaire Unique) flat rate
  Spain       — 19-28% progressive on crypto gains

HIGH friction:
  South Korea — 20% CGT >KRW 2.5M · VAUPA · no fan token exemption
  Italy       — 33% CGT (increased from 26% Budget 2025)
  South Africa — 18% effective CGT + CARF enforcement (HP-9 ACTIVE)

HIGHEST friction:
  Japan       — 55% effective rate (income tax treatment) ·
                reducing to 20% flat from January 2028

STATUTORY SUPPRESSOR (demand signal: MAXIMUM SUPPRESSION):
  Russia      — 282-FZ · fan tokens structurally excluded in statute

BINARY GATE — UNCONFIRMED:
  Pakistan    — PVARA · Path C ambiguous · NOC unconfirmed

AGENT RULE:
  Apply friction tier as a demand signal modifier for each
  jurisdiction's holder base. LOW friction = positive signal
  multiplier. STATUTORY SUPPRESSOR = zero demand signal from
  that jurisdiction — do not model residual demand.
  Never apply 17.5% Brazil CGT — LAPSED October 2025.
  Japan 20% flat rate takes effect January 2028 — not current.
```

---

## Multi-jurisdiction signal framework

```
AGENT RULE — MULTI-JURISDICTION REASONING:

  STEP 1: Identify which jurisdictions are material to the fan token signal
    Where are the primary token holders and traders located?

  STEP 2: Check Tier classification for each material jurisdiction
    Tier 1: apply binary authorisation gate before applying signal
    Tier 2: note developing framework, flag uncertainty
    Tier 3: apply demand suppressor — restricted or tax-impaired market
    Tier 4: treat as unregulated, no framework signal

  STEP 3: Apply gates independently per jurisdiction
    Never conflate EU MiCA status with UK FCA status
    Never conflate Singapore MAS status with Japan FSA status
    Each jurisdiction is a separate gate

  STEP 4: Sum adjusted signals across jurisdictions
    UK gate CLOSED + EU gate OPEN ≠ full demand signal
    Each closed gate suppresses the demand from that market only
    Gate closure always takes precedence over CHZ macro regime modifier

  MACRO INTERACTION:
    CHZ CAPITULATION regime (×0.70) applies globally.
    Example: UK gate OPEN + CAPITULATION ×0.70 = UK signal at ×0.70
    Example: UK gate CLOSED + CAPITULATION ×0.70 = UK signal suppressed
    Gate closure always takes precedence over regime modifier.
```

---

## International standard-setting bodies

```
BODIES WHOSE STANDARDS SHAPE NATIONAL FRAMEWORKS:

  FATF (Financial Action Task Force)
    Travel Rule: VASPs must share originator/beneficiary information
    for crypto transfers — applies globally to compliant VASPs
    Relevance: fan token platforms must comply or face de-banking risk
    Non-compliance = structural infrastructure risk, not demand signal

  FSB (Financial Stability Board)
    Global regulatory framework for crypto-asset activities (2023)
    Adopted as baseline by G20 jurisdictions
    Not a regulator — standard-setter only

  BIS / Basel Committee
    Capital requirements for banks holding cryptoassets (active Jan 2025)
    Group 2 assets (most cryptoassets including fan tokens): 1,250% risk weight
    Limits bank custody and balance sheet exposure to fan tokens
    Structural friction on institutional fan token custody globally

  IOSCO
    Crypto and digital asset policy recommendations
    Influences securities regulators globally on token classification

  AGENT RULE:
    FATF Travel Rule compliance is a prerequisite for compliant
    fan token platform operation globally.
    BIS capital rules create structural friction on institutional
    fan token custody — not a demand signal, a supply-side constraint.
```

---

## Tier upgrade monitoring

```
TIER UPGRADE SIGNALS — flag to Strategy & Brainstorm:

  A jurisdiction moves Tier 2 → Tier 1 when:
    A comprehensive authorisation framework is enacted (not just proposed)
    The framework explicitly covers cryptoasset trading platforms
    Enforcement begins (not just consultation)

  A jurisdiction moves Tier 3 → Tier 2 when:
    A formal regulatory framework or licensing regime is proposed
    Tax treatment is materially reformed (e.g. India reducing 30% rate)

  AGENT RULE:
    Tier upgrades are library events — bring to Strategy & Brainstorm
    for scoping before updating any library file.
    Do not update Tier classification based on SMI briefing alone —
    confirmation from official regulatory source required.
```

---

## MIND DIMENSIONS

| Dimension | Status | Notes |
|-----------|--------|-------|
| Intelligence | ACTIVE | Global regulatory map: four-tier jurisdiction classification |
| Reasoning | ACTIVE | Multi-jurisdiction gate reasoning, four-step framework |
| Context | ACTIVE | Jurisdiction-specific regulatory context per tier |
| Memory | ACTIVE | Tier classification and gate mechanics per jurisdiction |
| Judgment | ACTIVE | Tier 1 binary gate vs Tier 2 uncertainty vs Tier 3 suppressor |
| Attention | ACTIVE | Multi-jurisdiction signals require parallel independent gate checks |
| Communication | ACTIVE | Output always states: jurisdiction, tier, gate status |
| Verification | ACTIVE | Official regulator source required before Tier upgrade |
| Learning | ACTIVE | Tier classifications update as frameworks mature |
| Integration | ACTIVE | Integrates with all jurisdiction-specific regulatory files |
| Calibration | EMERGING | Global calibration developing as regimes enter enforcement |
| Adaptation | ACTIVE | Tier 2 jurisdictions may move to Tier 1 — monitor via SMI |
| Ethics | ACTIVE | Never assume compliance — always verify via official register |
| Transparency | ACTIVE | Always state: tier, gate status, jurisdiction source |

---

## Compatibility

**UK regime:** `macro/regulatory/uk-cryptoasset-regime.md`
**EU MiCA:** `macro/regulatory/eu-mica.md` · `macro/regulatory/eu.md`
**US framework:** `macro/macro-regulatory-sportfi.md`
**South Korea:** `macro/regulatory/south-korea.md`
**Singapore:** `macro/regulatory/singapore.md`
**Hong Kong:** `macro/regulatory/hong-kong.md`
**South Africa:** `macro/regulatory/south-africa-sars.md`
**Brazil:** `macro/regulatory/brazil.md`
**Russia:** `macro/regulatory/russia.md`
**Equity Token:** `equity-token/socios-equity-token.md`
**CHZ macro regime:** `macro/macro-crypto-market-cycles.md`
**Fan token registry:** `fan-token/registry/complete-registry.md`

---

*SportMind v4.6.70 · MIT License · sportmind.dev*
*Enduring framework — global cryptoasset regulatory landscape.*
*All 14 Mind dimensions mapped. Last verified 2026-09-14.*
