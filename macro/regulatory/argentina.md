---
SPORTMIND STALENESS NOTICE
File:            macro/regulatory/argentina.md
Tier:            A — 30-day verification cycle
Last verified:   2026-09-21
Next check due:  2026-10-21
Status:          NEEDS UPDATE
Agent rule:      Re-verification attempted 2026-09-21. Primary sources (bcra.gob.ar,
                 cnv.gob.ar, afip.gob.ar/arca.gob.ar, conmebol.com, docs.chiliz.com,
                 chiliscan.com) not reachable from build environment. V1 (AFIP/ARCA
                 rename), V2 (cepo status), V3 (CGT), V4 (CNV), V5 (Bienes Personales
                 thresholds), V7 ($ARG token), V8 (club tokens) all COULD NOT VERIFY.
                 Treat all regulatory rates, legislative status, tax body names, and
                 exchange control status as indicative only. Flag to operator for
                 primary-source re-verification before applying to live analysis.
---

# Argentina Regulatory Framework

**Domain:** macro/regulatory/argentina.md
**Version:** v4.6.18
**Library Rule:** Six-Month Test PASSES · Proper Noun Test PASSES
**Scope:** Argentina fan token holder regulatory framework.
  Highest-priority regulatory gap in SportMind library.
  UNKNOWN CGT modifier in force — dual-signal structure applies.

---

## Critical Distinction — Dual-Signal Structure

```
AGENT RULE 1 — DUAL-SIGNAL: NEVER NET OR COLLAPSE:
  Argentina is the only jurisdiction in the library with a confirmed
  DUAL-SIGNAL demand modifier structure. The two signals MUST NEVER
  be netted against each other or collapsed into a single modifier.

  SIGNAL 1: INFLATION HEDGE AMPLIFIER (positive demand signal)
    Argentine holders structurally motivated to hold USD-denominated
    assets — including CHZ and fan tokens — as protection against
    severe peso devaluation. Apply independently of football signals.

  SIGNAL 2: EXCHANGE CONTROL SUPPRESSOR (negative demand signal)
    BCRA cepo cambiario restricts peso-to-foreign-currency conversion.
    CHZ is a foreign-currency-denominated asset. Argentine holders
    face structural friction acquiring CHZ via formal channels.
    Apply independently of the inflation hedge amplifier.

  RULE: Report both signals separately in every Argentine holder
  analysis. Never report a single netted modifier. Never suppress
  one signal because the other is present.
```

---

## Regulatory Bodies

```
BCRA — Banco Central de la República Argentina
  Authority: bcra.gob.ar
  Role: Foreign exchange authority
  Scope: Cepo cambiario (exchange controls) — primary structural
    barrier for Argentine retail holders acquiring CHZ via peso
  Scan cadence: WEEKLY
  Escalation: IMMEDIATELY on any cepo reform or removal

CNV — Comisión Nacional de Valores
  Authority: cnv.gob.ar
  Role: Securities regulator
  Scope: No specific fan token classification or guidance issued
  Scan cadence: MONTHLY

AFIP — Administración Federal de Ingresos Públicos
  Authority: afip.gob.ar
  Role: Tax authority
  Scope: Crypto reporting framework ACTIVE since 2020
    CGT rate for fan tokens: NOT CONFIRMED (UNKNOWN modifier in force)
    Bienes Personales: applies to crypto holdings as financial assets
  Scan cadence: ON SIGNAL
```

---

## Legal Status

```
CRYPTO LEGAL STATUS: Legal with restrictions

· Digital assets subject to AFIP reporting framework (active 2020+)
· Exchange controls (cepo cambiario) via BCRA — primary structural
  barrier for CHZ acquisition via formal peso channels
· Socios.com: ACCESSIBLE · no confirmed ban · moderate Argentine
  user base present
· Kayen Finance: ACCESSIBLE in principle
· MiCA: NOT APPLICABLE — Argentina is not an EU member state
· CNV: no specific fan token classification issued at library date
```

---

## Tax Treatment — UNKNOWN Modifier

```
CGT STATUS: UNKNOWN
  No confirmed enacted law on fan token CGT rate or disposal treatment.
  AFIP crypto reporting: ACTIVE since 2020 — reporting obligation
    confirmed · rate and treatment for fan tokens NOT confirmed.
  UNKNOWN modifier: ACTIVE — do not apply any other jurisdiction's
    CGT framework to Argentine holders.

BIENES PERSONALES (wealth tax):
  Applies to crypto holdings as financial assets.
  This is a holding tax, not a disposal tax — creates ongoing cost of
  holding rather than a disposal-event incentive.
  Additional demand suppressor layer above any CGT obligation.
  Apply as secondary suppressor for high-value Argentine holders.
  Retail holders below thresholds: negligible practical impact.
  Monitor AFIP for any Bienes Personales reform affecting crypto.

PESO INFLATION:
  Severe — one of the highest sustained inflation rates globally.
  Argentine holders are structurally motivated to hold
  USD-denominated assets as an inflation hedge.
  This is a demand AMPLIFIER operating independently of football
  signals, CGT status, and exchange control friction.

STANDING RULE:
  Until this file is superseded by confirmed enacted law from AFIP:
  Apply UNKNOWN CGT modifier to ALL Argentine holder tax analysis.
  Do not retire the UNKNOWN modifier without Tier 1 source confirmation
  from afip.gob.ar.
```

---

## Modifier 1 — Inflation Hedge Amplifier

```
MODIFIER: INFLATION HEDGE AMPLIFIER
SIGNAL DIRECTION: POSITIVE (demand amplifier)

Argentina has among the highest retail crypto adoption globally by
penetration — driven primarily by inflation hedge behaviour, not
speculative trading. Argentine retail holders use USD-denominated
crypto assets (including CHZ and fan tokens) as protection against
severe peso devaluation.

· Apply as a POSITIVE demand signal to ALL Argentine holder analysis
· Apply INDEPENDENTLY of football signals, PTG event status,
  or current exchange control restrictions
· Do NOT net against the exchange control suppressor
· Report alongside — never instead of — the exchange control suppressor
· Does not require any football catalyst to be active
· Active continuously as long as Argentine peso inflation remains severe

AGENT RULE 3 — INFLATION HEDGE AMPLIFIER:
  Apply positive demand signal to all Argentine holder analysis
  independently of football signals. Do not suppress due to regulatory
  uncertainty or the presence of the exchange control suppressor.
```

---

## Modifier 2 — Exchange Control Suppressor (Cepo Cambiario)

```
MODIFIER: EXCHANGE CONTROL SUPPRESSOR
SIGNAL DIRECTION: NEGATIVE (demand suppressor)

BCRA exchange controls (cepo cambiario) restrict formal peso-to-foreign-
currency conversion. CHZ is a foreign-currency-denominated asset.
Argentine holders face structural friction in acquiring CHZ via formal
channels — even when motivated to buy.

· Apply as a NEGATIVE demand signal operating independently of the
  inflation hedge amplifier
· Do NOT net against the inflation hedge amplifier
· Report alongside — never instead of — the inflation hedge amplifier
· Active as long as cepo cambiario remains in force via BCRA

MILEI GOVERNMENT REFORM DIRECTION:
  The Milei government is crypto-friendly in direction. Any cepo
  liberalisation or removal represents an IMMEDIATE demand amplifier
  for CHZ and all fan tokens with Argentine holder bases.

ESCALATION RULE:
  ESCALATE IMMEDIATELY if BCRA announces cepo reform, liberalisation,
  or removal. Do not wait for next scheduled briefing. Cepo removal
  would retire this suppressor and materially amplify the inflation
  hedge signal without the friction overlay.

AGENT RULE 4 — CEPO ESCALATION:
  Escalate IMMEDIATELY on any BCRA exchange control reform or removal.
  Cepo removal = immediate demand amplifier. Do not wait for next briefing.
```

---

## Modifier 3 — Bienes Personales Wealth Tax

```
MODIFIER: BIENES PERSONALES SUPPRESSOR
SIGNAL DIRECTION: NEGATIVE (secondary suppressor)

Argentina's Bienes Personales wealth tax applies to crypto holdings as
financial assets. Unlike CGT (applied on disposal), this is an ongoing
holding cost.

· Apply as a SECONDARY suppressor for high-value Argentine holders
· Retail holders below thresholds: negligible practical impact —
  do not apply to low-value holder segments
· Monitor AFIP for any Bienes Personales reform affecting crypto asset
  treatment — any reform could shift this modifier
· Does not replace the UNKNOWN CGT modifier — operates in addition to it
```

---

## Diaspora Modifier Stack — $ARG

```
$ARG has significant diaspora holder populations outside Argentina.
Each diaspora segment carries their RESIDENT jurisdiction's tax modifier —
NOT the Argentine UNKNOWN modifier.

RULE: When analysing $ARG aggregate demand, always decompose into:
  · Domestic Argentine holders: UNKNOWN CGT modifier (this file)
  · Diaspora segments: resident jurisdiction modifier (see below)
  Never apply a single-jurisdiction modifier to $ARG aggregate demand.

ITALIAN DIASPORA:
  Italy has the largest concentration of Argentine-heritage residents
  outside Argentina — estimated 3-5M Italians with Argentine ancestry.
  Apply Italian tax framework (Type C — approx. 26% flat CGT) to
  Italian-resident $ARG holders. MODERATE-HIGH friction applies.
  ⚠️ CONFLICT FLAG: italy.md now states 33% CGT (raised from 26% per
  Decreto Legislativo 199/2025, effective 2026-01-01). Rate here not
  updated pending patch of italy.md.

GERMAN DIASPORA:
  Apply Type A framework (0% CGT after 12-month holding period).
  LONG HOLD BIAS applies to German-resident $ARG holders.

SPANISH DIASPORA:
  Apply Type D framework (19-28% progressive CGT — Impuesto sobre
  la Renta). PROGRESSIVE FRICTION applies.

AGENT RULE 5 — $ARG COMPOSITE MODIFIER:
  Decompose $ARG demand into:
    Domestic Argentine (UNKNOWN) +
    Italian diaspora (Type C) +
    German diaspora (Type A) +
    Spanish diaspora (Type D)
  Never apply single-jurisdiction modifier to $ARG aggregate.
```

---

## $ARG Token Profile

```
TOKEN: $ARG (Argentina National Team)

WC2026 RECORD:
  · Result: Finalist — lost in final
  · PTG burns: 7 confirmed
  · Trophy premium: NOT ELIGIBLE — finalist only (no WC win)
  · Finalist narrative: STRONG holder engagement signal without
    trophy premium uplift

POST-WC2026 STATUS:
  · PTG burns in active decay window post-WC2026
  · WC burn cycle CLOSED — no WC trophy premium applicable

NEXT PTG WINDOWS:
  · Copa América 2027 (USA) — next PTG window for $ARG
    Confirm eligibility via burn-to-glory-framework.md before encoding
  · WC2030 qualifying: begins 2027

BURN TO GLORY:
  · No WC win = no WC burn cycle
  · Copa América win would trigger if eligible — verify framework

FAN BASE:
  · 45M+ estimated global (tournament window)
  · Primary holder market: Argentina (UNKNOWN CGT modifier)
  · Significant diaspora segments: Italy · Germany · Spain
    (apply diaspora modifier stack above)
```

---

## Domestic Club Token Status

```
STATUS: No Argentine club tokens currently active on Chiliz Chain

MONITOR TARGETS:
  · River Plate (Club Atlético River Plate)
  · Boca Juniors (Club Atlético Boca Juniors)

ESCALATION RULE:
  Any confirmed Chiliz Chain partnership with an Argentine Primera
  División club → escalate IMMEDIATELY to Strategy & Brainstorm.

  Boca Juniors or River Plate partnership = HIGHEST-VALUE South
  American club fan token announcement possible in library.
  Treat as immediate Tier 1 signal on confirmation.

AGENT RULE 6 — BOCA/RIVER ESCALATION:
  Any Chiliz partnership with River Plate or Boca Juniors = immediate
  Tier 1 signal. Escalate to Strategy & Brainstorm immediately.
  Do not wait for next scheduled briefing.
```

---

## Milei Government Reform Signal

```
GOVERNMENT DIRECTION: CRYPTO-FRIENDLY

Key reform vectors to monitor:

CEPO CAMBIARIO REFORM (BCRA):
  Any liberalisation or removal of exchange controls → immediate
  demand amplifier for CHZ and all fan tokens with Argentine holder
  bases. Retires exchange control suppressor if fully removed.
  WEEKLY BCRA scan watching specifically for this signal.
  ESCALATE IMMEDIATELY — do not wait for next briefing.

AFIP TAX FRAMEWORK CLARIFICATION:
  Any AFIP confirmation of an enacted fan token CGT rate or disposal
  treatment → immediate opportunity to retire UNKNOWN modifier and
  encode the actual confirmed rate into this file and into
  fan-token/holder-tax-framework.md.
  Bring to Build Chat immediately with Tier 1 source link (afip.gob.ar).

DIRECTIONAL NOTE:
  Milei government deregulation trajectory favours crypto normalisation.
  Do not extrapolate this into a confirmed framework change — only
  enacted legislation or confirmed AFIP/BCRA guidance triggers a
  modifier update. Direction noted; confirmed change required to act.
```

---

## Open Items / Escalation Rules

```
OPEN ITEMS:

1. $ARG WC2026 PTG burn count: 7 burns cited — VERIFY from
   chiliscan.com before using in calibration. Confirm each burn
   event and round before encoding in calibration records.

2. Copa América 2027 PTG eligibility: confirm with
   burn-to-glory-framework.md before encoding any pre-event signal.

3. UNKNOWN CGT modifier: retire ONLY when AFIP confirms enacted
   fan token tax rate via Tier 1 source (afip.gob.ar). No other
   source qualifies. Do not retire on media reports alone.

4. Bienes Personales threshold: confirm current exemption thresholds
   before applying to specific holder value segments.

ESCALATE IMMEDIATELY:
  · BCRA cepo cambiario reform, liberalisation, or removal
  · AFIP enacted fan token CGT rate confirmation
  · River Plate Chiliz partnership confirmation
  · Boca Juniors Chiliz partnership confirmation
  · Any Primera División club Chiliz partnership
```

---

## 6 Agent Rules

```
RULE 1 — DUAL-SIGNAL: NEVER NET OR COLLAPSE:
  Report INFLATION HEDGE AMPLIFIER and EXCHANGE CONTROL SUPPRESSOR
  separately in every Argentine holder analysis. Never net, collapse,
  or report a single combined modifier. Both signals are structurally
  independent and must be treated as such.

RULE 2 — UNKNOWN CGT:
  Apply UNKNOWN modifier to all Argentine holder tax analysis until
  AFIP confirms enacted rate via Tier 1 source (afip.gob.ar).
  Do not apply any other jurisdiction's CGT framework to Argentine holders.

RULE 3 — INFLATION HEDGE AMPLIFIER:
  Apply positive demand signal to all Argentine holder analysis
  independently of football signals, PTG event status, or regulatory
  uncertainty. Do not suppress due to exchange control friction.
  This signal is continuously active as long as peso inflation persists.

RULE 4 — CEPO ESCALATION:
  Escalate IMMEDIATELY on any BCRA exchange control reform or removal.
  Cepo removal = immediate demand amplifier and retires the exchange
  control suppressor. Do not wait for next scheduled briefing.

RULE 5 — $ARG COMPOSITE MODIFIER:
  Decompose $ARG demand into domestic Argentine (UNKNOWN) + Italian
  diaspora (Type C) + German diaspora (Type A) + Spanish diaspora
  (Type D). Never apply single-jurisdiction modifier to $ARG aggregate.
  Apply each diaspora segment's resident jurisdiction modifier.

RULE 6 — BOCA/RIVER ESCALATION:
  Any Chiliz partnership with River Plate or Boca Juniors = immediate
  Tier 1 signal. Escalate to Strategy & Brainstorm immediately.
  Do not wait for any scheduled review.
```

---

## MIND DIMENSIONS

| Dimension | Status | Notes |
|---|---|---|
| Intelligence (1) | ACTIVE | Dual-signal demand modifier structure as regulatory intelligence |
| Reasoning (2) | ACTIVE | Causal reasoning: cepo friction vs inflation hedge → dual independent signals |
| Context (3) | ACTIVE | Argentine macro context (peso inflation, exchange controls, Milei direction) |
| Memory (4) | ACTIVE | UNKNOWN CGT modifier, dual-signal rules, diaspora stacks, escalation conditions |
| Judgment (5) | ACTIVE | Composite modifier weighting for $ARG multi-market demand decomposition |
| Attention (6) | ACTIVE | BCRA weekly cepo scan · AFIP enacted rate trigger · Boca/River escalation |
| Communication (7) | ACTIVE | Dual-signal notation — both signals reported separately in every output |
| Verification (8) | ACTIVE | PTG burn count verification via chiliscan.com before calibration use |
| Learning (9) | ACTIVE | UNKNOWN modifier retires when AFIP confirms — framework updates immediately |
| Integration (10) | ACTIVE | Cross-layer: inflation hedge + cepo suppressor + CDI + macro regime = composite |
| Calibration (11) | ACTIVE | Modifier values to calibrate via verified calibration records for $ARG |
| Adaptation (12) | ACTIVE | Milei reform trajectory — cepo removal and AFIP clarification both trigger updates |
| Ethics (13) | ACTIVE | No tax advice framing — demand modifier framework only |
| Transparency (14) | ACTIVE | UNKNOWN modifier explicitly stated · open items clearly flagged |
| Execution (15) | ACTIVE | $ARG composite decomposition executed per Rule 5 in every analysis |
| Collaboration (16) | ACTIVE | Diaspora modifier stack requires cross-file coordination with holder-tax-framework.md |

---

## COMPATIBILITY

- intelligence/country-scan/argentina.md — Argentine market and holder context
- fan-token/holder-tax-framework.md — Rule 9 UNKNOWN modifier · USA and Argentina entries
- macro/regulatory/south-africa-sars.md — sibling regulatory file (structure reference)
- macro/regulatory/uk-cryptoasset-regime.md — sibling regulatory file
- macro/global-regulatory-landscape.md — four-jurisdiction overview
- fan-token/fan-holder-behaviour.md — holder behaviour framework
- burn-to-glory-framework.md — $ARG Copa América 2027 PTG eligibility
- core/signal-classification-framework.md — compliance enforcement = Type 3 Structural
- fan-token/registry/complete-registry.md — $ARG registry entry and token verification

© 2026 SportMind
