# South Africa — SARS Crypto Tax Framework

**Domain:** macro/regulatory/south-africa-sars.md
**Version:** v4.1.40
**Library Rule:** Six-Month Test PASSES · Proper Noun Test PASSES
**Scope:** South African Revenue Service (SARS) crypto asset tax
framework. Draft Guide to Taxation of Crypto Assets (July 1 2026),
CARF implementation, and $SAFA fan token holder implications.
TFM4 deadline protocol: comment period closes August 31 2026.

---

## Regulatory Timeline

```
March 2026     — CARF (Crypto-Asset Reporting Framework) implemented
                 South Africa adopts OECD CARF for automatic
                 information exchange between tax authorities
July 1 2026    — SARS Draft Guide to Taxation of Crypto Assets
                 published. Opens public comment period.
August 31 2026 — Comment period closes (TFM4 T+0 deadline)
                 Post-comment: final guide expected · enforcement hardens
TBD 2026-2027  — Final guide published and enforcement begins
                 Crypto Revenue Augmentation Unit operational
```

**TFM4 DEADLINE PROTOCOL — ACTIVE:**
```
T-30 = August 1 2026   — ESCALATION ACTIVE · flag in every SMI briefing
T-7  = August 24 2026  — DAILY SCAN · check SARS website for late guidance
T+0  = August 31 2026  — COMMENT PERIOD CLOSES · enforcement trajectory confirmed
T+30 = September 30    — POST-DEADLINE MONITOR · confirm final guide timeline
```

---

## Crypto Asset Classification

```
CLASSIFICATION: Intangible assets
  NOT currency · NOT cash equivalents

TAX TREATMENT:
  Income tax: applies if crypto is received as income or traded
    as a business activity
  Capital Gains Tax (CGT): applies on disposal of crypto assets
    held as investments
  Determination: based on facts and circumstances of each holder
    (trader vs investor distinction)

DISPOSAL EVENTS (tax-triggering):
  · Sale of crypto asset for fiat
  · Exchange of one crypto asset for another
  · Use of crypto to purchase goods or services
  · Gifting of crypto assets

FAN TOKEN SPECIFIC:
  Purchasing a fan token: not a disposal (acquisition)
  Selling a fan token: disposal — CGT or income tax applies
  Receiving fan token rewards: may constitute income
  PTG treasury burns: tax treatment NOT addressed in July 2026
    draft guide — TBC, monitor final guide
```

---

## CARF Implementation

```
WHAT CARF IS:
  Crypto-Asset Reporting Framework (OECD). Automatic exchange of
  information between tax authorities about crypto-asset holdings
  and transactions of foreign tax residents.

SOUTH AFRICA IMPLEMENTATION:
  Effective: March 2026
  First reporting period: March 2026 to February 2027
  Reporting deadline: by end of March 2027

WHAT IS REPORTED:
  · Crypto-asset exchanges (trades, sales, conversions)
  · Crypto-asset transfers (in and out)
  · Fiat-to-crypto and crypto-to-fiat transactions
  · Wallet holdings (in some implementations)

WHO REPORTS:
  Crypto-Asset Service Providers (CASPs) operating in South Africa
  or with South African customers must report to SARS.

KEY IMPLICATION — OFFSHORE HOLDINGS NO LONGER SHELTERED:
  South African tax residents with crypto assets held on overseas
  platforms are now subject to automatic reporting via CARF.
  The "offshore = unreported" assumption is no longer valid.

AGENT RULE: CARF compliance is mandatory for CASPs serving South
  African residents. Non-compliant platforms face enforcement risk.
  This may affect platform access for South African fan token holders.
```

---

## Crypto Revenue Augmentation Unit

```
STATUS: ESTABLISHED (2026)
FUNCTION: Dedicated SARS unit for crypto asset compliance and audit
ENFORCEMENT SIGNALS:
  · Unit establishment = enforcement intent, not passive monitoring
  · Targeted audits of high-value crypto traders expected
  · Cross-referencing bank records, exchange data, CARF reports
  · Penalties for non-disclosure apply

AGENT RULE: SARS enforcement capability is now materially higher
  than in prior years. South African crypto holders who have not
  declared assets or gains face audit exposure. This may create
  compliance-driven selling pressure in South African markets.
```

---

## $SAFA Fan Token — Holder Implications

```
$SAFA STATUS: Confirmed active Chiliz Chain fan token
  (South African Football Association)
  WC2026: eliminated Round of 32 · 1 PTG burn confirmed

SOUTH AFRICAN HOLDER TAX IMPLICATIONS:
  Purchase of $SAFA: acquisition — no tax event
  Sale of $SAFA: disposal — CGT or income tax applies
    CGT rate for individuals: effective rate up to 18%
    Income tax: up to 45% for high-income holders
  Exchange of $SAFA for other crypto: disposal — tax applies
  Holding $SAFA: no annual tax — only on disposal

PTG BURN TAX TREATMENT:
  The July 2026 draft guide does NOT address PTG burn events.
  A PTG burn reduces token supply from treasury (not holder
  wallets directly). Direct holder wallet impact is through
  secondary market price effects, not a disposal.
  Monitor final guide for any specific treatment of supply events.

COMPLIANCE-DRIVEN SELLING PRESSURE MODIFIER:
  SARS enforcement + CARF reporting creates compliance-driven
  pressure on South African $SAFA holders to:
  · Declare existing holdings (may prompt realisation)
  · Avoid large undisclosed positions
  This is a market-structure modifier for South African demand.
  Apply as a LOW-MEDIUM negative modifier on $SAFA South African
  retail demand in periods of SARS enforcement activity.
```

---

## 5 Agent Rules

```
RULE 1 — TFM4 PROTOCOL ACTIVE:
  Comment period closes August 31 2026. Apply TFM4 at T-30
  (August 1) — this is the escalation date. Flag in every SMI
  briefing from August 1. Daily scan from August 24.

RULE 2 — CARF = OFFSHORE NO LONGER SHELTERED:
  South African crypto holders cannot use offshore exchanges
  to avoid SARS reporting. CARF automatic exchange means
  SARS receives data from compliant foreign exchanges.
  Apply as a compliance risk modifier for South African markets.

RULE 3 — PTG BURN TREATMENT UNKNOWN:
  The draft guide does not address PTG burn events. Do not
  speculate about tax treatment. Monitor final guide.
  Until confirmed: PTG burns are not treated as disposal events
  for holders (no wallet-level reduction for holders).

RULE 4 — COMPLIANCE-DRIVEN SELLING MODIFIER:
  SARS enforcement activity is a demand-suppressing modifier
  for South African retail crypto holders. Apply as LOW-MEDIUM
  negative modifier on South African domestic fan token demand
  during enforcement-active periods.

RULE 5 — POST-COMMENT ENFORCEMENT HARDENS:
  After August 31, the comment period closes and enforcement
  trajectory becomes clearer. Post-deadline: update this file
  with final guide findings. Enforcement intensity is expected
  to increase materially after final guide publication.
```

---

## MIND DIMENSIONS

| Dimension | Status | Notes |
|---|---|---|
| Intelligence (1) | ACTIVE | SARS crypto classification and tax treatment intelligence |
| Reasoning (2) | ACTIVE | Causal: CARF → offshore reporting → compliance pressure → selling |
| Context (3) | ACTIVE | South African regulatory context, enforcement timeline |
| Memory (4) | ACTIVE | CARF first reporting period, TFM4 deadline dates |
| Judgment (5) | ACTIVE | Compliance modifier weighting, enforcement risk assessment |
| Attention (6) | ACTIVE | TFM4 T-30 escalation on August 1 — active attention required |
| Communication (7) | ACTIVE | TFM4 protocol communication, agent rule format |
| Verification (8) | ACTIVE | SARS official portal verification, CARF reporting confirmation |
| Learning (9) | ACTIVE | Post-comment final guide will update this file |
| Integration (10) | ACTIVE | Cross-references $SAFA, CARF, global regulatory landscape |
| Calibration (11) | ACTIVE | Compliance-driven selling modifier to be calibrated post-enforcement |
| Adaptation (12) | ACTIVE | File must update post-August 31 with final guide findings |
| Ethics (13) | ACTIVE | Tax compliance obligations — transparency about CARF scope |
| Transparency (14) | ACTIVE | PTG burn treatment explicitly flagged as unknown |
| Execution (15) | ACTIVE | TFM4 deadline protocol — T-30/T-7/T+0/T+30 execution |
| Collaboration (16) | NOT APPLICABLE | — |

---

## COMPATIBILITY

- macro/global-regulatory-landscape.md — South Africa Tier 2 regulatory status
- core/temporal-reasoning-framework.md — TFM4 regulatory deadline protocol applied
- fan-token/registry/complete-registry.md — $SAFA token profile
- macro/fan-adoption-intelligence.md — South African market demand modifiers

© 2026 SportMind
