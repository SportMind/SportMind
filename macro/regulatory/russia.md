...
```
RUSSIAN CRYPTO REGULATORY FRAMEWORK — EVOLUTION:

Crypto payment ban — effective April 2020:
  Bank of Russia prohibited use of crypto assets for domestic
  payments of goods and services.
  Ruble remains sole lawful domestic currency.
  Payment ban maintained in Bill 1194918-8 — not lifted.

Law on Digital Financial Assets (DFA Law) — effective January 1 2021:
  First dedicated digital asset legislation in Russia.
  Classified digital financial assets as property.
  Prohibited crypto as a means of payment.
  Did not create a trading framework — grey market persisted.

Crypto Mining Law — signed August 2024:
  Legalised cryptocurrency mining in Russia.
  Permitted international crypto payments through an experimental
  legal regime.
  First step toward formalised crypto framework.
...
DIRECTION OF TRAVEL:
  ...
  to grudging tolerance (2021-2023) to active legalisation
  (2024-2026). The primary driver is sanctions bypass —
  cross-border trade settlement in crypto formalises what
  grey-market networks were already doing. Domestic crypto use
  remains strictly controlled.
```

---

## Current Regulatory Framework (effective September 1 2026)

```
CRYPTO PROPERTY STATUS:
  Bill 1194918-8 classifies crypto assets as property (имущество)
...
DOMESTIC PAYMENT BAN (MAINTAINED):
  Ruble remains sole lawful currency for domestic transactions.
  Crypto cannot be used for goods and services payments in Russia.
  Advertising crypto for domestic payment use is prohibited.
  Structural holder behaviour modifier: Russian fan token holders
  are investment/speculation only — same constraint as Turkey
  but with additional platform access restrictions.

CROSS-BORDER TRADE SETTLEMENT (LEGALISED):
  Russian companies may use crypto to settle international trade
  transactions — explicit sanctions bypass mechanism.
  This does not affect fan token holder demand directly but
  signals the government's primary motivation for legalisation.

BANK OF RUSSIA LICENSING FRAMEWORK:
  Five licensed participant categories from September 1 2026:
    1. Exchanges (биржи)
    2. Brokers (брокеры)
    3. Custodians/depositories (депозитарии)
    4. Management companies (управляющие компании)
    5. Exchangers (обменники)
  Banks barred from routing payments to unlicensed foreign
...
DIGITAL RUBLE (PARALLEL LAUNCH):
  ...
  September 1 2026 — same date as crypto framework.
  Digital ruble is a CBDC — separate from crypto assets.
  Signals Russia's broader digital currency strategy.
  Not directly relevant to fan token analysis but provides
  structural context for Russian digital asset policy.

CGT / TAX FRAMEWORK:
  No specific crypto capital gains tax rate confirmed in
  Bill 1194918-8 — tax provisions left to Federal Tax Service
  implementing guidance.
  General income tax rates (13-15% for individuals) theoretically
  apply to crypto gains as property income.
  No specific enforcement mechanism for fan token gains confirmed.
...
```
CLASSIFICATION: Tsifrovoy aktiv (цифровой актив — digital asset)
  Bill 1194918-8 classifies crypto assets as property under
  Russian civil code. Fan tokens as Chiliz Chain utility tokens
  fall within the general digital asset definition.
...
## Bidirectional Platform Access Restrictions

```
DIRECTION 1 — RUSSIAN LAW RESTRICTS OUTBOUND ACCESS:
  From September 1 2026: banks barred from routing payments
  to unlicensed foreign platforms.
  Socios.com / Chiliz: not a Bank of Russia-licensed CASP.
...
DIRECTION 2 — EU SANCTIONS RESTRICT INBOUND ACCESS:
  EU 20th sanctions package (effective May 2026): EU residents
  prohibited from transacting with Russian crypto service
  providers.
  Socios.com / Chiliz: EU-licensed (MFSA Malta authorisation —
  MiCA passporting). Cannot legally serve Russian-resident
  users under EU sanctions framework.
  Western-licensed fan token platforms are structurally blocked
  from Russian user onboarding.
  This is a demand signal modifier: Russian fan token demand
  cannot easily flow through mainstream Western platforms.

COMBINED STRUCTURAL EFFECT:
  Russian-resident fan token holders face restrictions from
  both directions simultaneously:
  · Russian law blocks payment routing to unlicensed foreign
    platforms and caps investment via licensed ones
  · EU sanctions prevent Western platforms from serving
    Russian users
  This makes Russia the most structurally isolated holder
  jurisdiction currently documented in SportMind.
  Do not model Russian domestic demand as equivalent to other
  high-friction jurisdictions (Italy · Brazil · Turkey) —
  the access restriction layer is qualitatively different.

AGENT RULE 2 — BIDIRECTIONAL ACCESS RESTRICTION:
  Apply bidirectional access restriction modifier to all
  Russian-resident fan token holder analysis.
  Russian law + EU sanctions = structural platform isolation.
  Russian holder demand signals for any Chiliz fan token
  carry elevated uncertainty and access friction not present
  in any other jurisdiction in the SportMind library.
```
...
RULE 2 — BIDIRECTIONAL ACCESS RESTRICTION MODIFIER:
  ...
  Russian law and EU sanctions simultaneously.
  Apply maximum access restriction modifier — qualitatively
  different from other high-friction jurisdictions.

RULE 3 — DOMESTIC PAYMENT BAN ACTIVE:
  Russian holders cannot use fan tokens for domestic payments.
  Investment and speculation only — same structural constraint
  as Turkey but compounded by platform access restrictions.

RULE 4 — NON-QUALIFIED INVESTOR CAP:
  R$300,000 (~$3,800) annual purchase cap for non-qualified
  Russian investors via licensed intermediaries.
  If fan tokens are confirmed eligible: this cap structurally
  suppresses Russian domestic demand relative to other markets.
  Apply as demand suppression modifier if eligibility confirmed.

RULE 5 — CGT UNKNOWN:
  No specific crypto CGT rate confirmed in Bill 1194918-8.
  General income tax (13-15%) theoretically applies.
  No enforcement mechanism for fan token gains confirmed.
  Apply UNKNOWN modifier for Russian domestic tax friction.

RULE 6 — NO RUSSIAN CLUB TOKENS IN LIBRARY:
  No confirmed Russian club fan tokens on Chiliz Chain
  as of 2026-07-31. This file covers Russian-resident
  holders of any Chiliz fan token — not Russian-club-specific
...
RULE 8 — SANCTIONS COMPLIANCE NOTE:
  ...
  Agents must not facilitate transactions that violate applicable
  sanctions regimes. Russian-resident holder analysis is for
...
```
NO RUSSIAN CLUB FAN TOKENS CONFIRMED:
  As of 2026-07-31, no Russian football club holds a confirmed
  active Chiliz fan token partnership.
  Major Russian clubs (not confirmed as Chiliz partners):
    CSKA Moscow · Zenit St Petersburg · Spartak Moscow ·
    Lokomotiv Moscow — none confirmed.
  Do not create CDI files or signal analysis without verified
  Chiliz partnership.

RUSSIAN-RESIDENT HOLDERS OF ANY CHILIZ TOKEN:
  This file applies to Russian-resident holders of any active
  Chiliz fan token. Key tokens with potential Russian holder
  base (verify on fantokens.com before applying):
    $GAL (Galatasaray) — Turkish token with potential Russian
      diaspora / sports interest holder cohort
    $INTER · $JUV · $ACM — Italian club tokens with potential
      Russian holder base
    $PSG · $BAR — high-profile tokens with global holder reach
  Apply Russian framework to any of the above where Russian
  resident holder demand is being assessed.

MONITORING:
  If any Russian football club launches a Chiliz fan token:
  escalate immediately to Strategy & Brainstorm.
  This would be a HIGH PRIORITY library event requiring a
  dedicated CDI file and Russian framework application.
```
...
| Jurisdiction | CGT rate | Threshold | Hold period relief | Visibility (2026) | Access restrictions |
|---|---|---|---|---|---|
| Italy | 33% | None | None | Full — DAC8 | None beyond tax |
| France | 30% flat | None | None | Partial | None beyond tax |
| UK | 20% higher rate | £3,000 exempt | None | Partial | None beyond tax |
| Brazil | 17.5% flat (MP) | None | None | High — DeCripto | None beyond tax |
| South Africa | 18% max CGT | None | None | Partial — CARF | None beyond tax |
| Germany | 0% after 12m | €600 annual | YES — 12m | Partial | None beyond tax |
| Turkey | UNKNOWN | N/A | N/A | Moderate — SPK | Payment ban |
| Russia | UNKNOWN | N/A | N/A | Low — isolated | Bidirectional: domestic law + EU sanctions |

Russia is the most structurally isolated holder jurisdiction in
the SportMind library. Restrictions operate from both directions
simultaneously. No other jurisdiction in the library has confirmed
bidirectional access restrictions.
...
Fan token CGT treatment under Russian property tax law
  Status: UNRESOLVED
  Action: Monitor Federal Tax Service implementing guidance.
    Escalate if specific fan token or utility token guidance issued.

Socios.com / Chiliz Bank of Russia CASP licence
  Status: NOT EXPECTED — verify
  Action: If Chiliz applies for Bank of Russia licence:
    escalate immediately — HIGH PRIORITY signal.

Russian club fan token launches
  Status: MONITOR
  Action: Escalate immediately if any Russian club announces
    Chiliz partnership.

EU sanctions expansion — crypto provisions
  Status: MONITOR
  Action: Check EU sanctions packages for expanded crypto
...
PRIMARY SOURCES:
  ...
  EU 20th sanctions package — May 2026 (crypto CASP restrictions
    on Russian service providers)

SECONDARY SOURCES (cross-check only):
  Bitcoin News — July 21 2026 (Duma passage + provisions)
  Bitcoin Magazine — July 21 2026 (landmark law analysis)
  TechTimes — July 21 2026 (sanctions bypass analysis)
  Cryptopolitan — July 2026 (September 1 enforcement date)
  CryptoTicker — July 28 2026 (US CLARITY Act comparison)
...
Compatible with:
macro/regulatory/eu-mica.md — EU sanctions provisions
macro/regulatory/turkey.md — comparable payment ban structure
macro/regulatory/italy.md — cross-jurisdiction holder comparison
macro/regulatory/brazil.md — cross-jurisdiction holder comparison
