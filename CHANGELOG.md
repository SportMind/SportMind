...
### Changed
- CHANGELOG.md: Missing entry v4.1.53 added (CHANGELOG gap fill
  covering v4.1.49 through v4.1.52).
...
### Changed
- CHANGELOG.md: Missing entries v4.1.23 and v4.1.24 added. Complete file
  restored — prior version was overwritten with patch document. All entries
  from original history retained.
...
### Changed
- index.html (Website Pass 3): Version stamp v4.1.12 → v4.1.24. Stats block:
  760→762 files, 96%→9/9 WC2026 100% accuracy, 21 sports validated → 42
  sports covered. Empirical foundation: 96% hero number replaced with 9/9
  WC2026 · 100% accuracy · perfect record. Leaderboard: 96% removed from
  maintainer row. Tier A sports: basketball, cricket, MotoGP removed (no
  confirmed active Chiliz fan tokens). Rugby and Esports added (confirmed
  active: $SFP, $SHARKS, $ALL, $DOJO, $MIBR, $OG, $TH). Fraud warning:
  "World Cup 2026" → "major international tournaments". Layer counts:
  athlete 45→50, fan-token 21→90, market 48→51.
...
### Added
- market/club-intelligence/avl.md: Aston Villa club intelligence. UCL 2026-27
  confirmed (Europa League winners). Major squad rebuild: creative midfield
  departed, two new arrivals, different archetype. Long-term absence compounds
  transition. TRANSITION gate ACTIVE. CDI horizon SHORT — reassess post
  pre-season August-September 2026.

### Changed
- macro/partnerships.md: US college sports market entry confirmed July 2026.
  Chiliz distribution partnership with major US sports media firm. Five initial
  tokens (SEC and Big Ten conferences), up to 30 college teams in 12 months.
  NIL integration: fan token revenues support student-athlete NIL programs —
  utility layer absent from European club model. Performance-linked tokenomics
  confirmed in principle; mechanic TBC — do not assume Burn to Glory or Fan
  Token Play until confirmed.
- fan-token/use-cases.md: NIL Revenue Integration section added. US college
  model (fan → club → athlete) structurally distinct from European model
  (club → fan). Three agent rules: separate demand model, mechanic TBC,
  European frameworks do not directly apply.
...
### Changed
- README.md: Version badge 3.97.46 → 4.1.22. Calibration badge: 96% claim
  removed, WC2026 9/9 100% added. Fan tokens badge: 63 active · 85 registered.
  File counts: 679→762 files, 460→542 markdown, 129→137 records. Macro layer
  count: 9→20 docs. Calibration section updated with WC2026 9/9 100% record.
...
### Changed
- CHANGELOG.md: 16 missing entries added covering v4.1.6 through v4.1.21
  (WC2026 build period and GitHub documentation audit Pass 1).
...
### Changed
- SECURITY.md: Skill hash count (273) generalised to "all skill files" in two
  occurrences. Duplicate Changelog section removed. Calibration provenance note
  added. Trust tier table: "Expert/Senior on leaderboard" → "established
...
### Changed
- OVERVIEW.md: Opening description updated. Key Numbers corrected: 762 files,
  542 markdown, 137 records. 96% claim removed, WC2026 9/9 100% perfect added.
  Footer version: v3.97.41 → v4.1.19. SMI description updated. Direction format
  note added. Founding Calibrators noted.
...
### Changed
- QUICKSTART.md: Version updated: 3.40.0 → 4.1.18. Direction format note added.
  Agent prompt count removed from "Where to go next" table.
...
### Changed
- MCP-SERVER.md: Health check version: 3.30.0 → 4.1.17. Malformed version
  strings removed from two section headers. "The ten tools" → "Core tools" with
  five/five split note. sportmind_pre_match added to sequencing. Registry count:
  84→85. Model string: claude-sonnet-4-20250514 → claude-sonnet-4-6.
...
### Changed
- FIRST-RECORD-CHALLENGE.md: Count → 137. Founding Calibrators noted. Step 2
  rewritten with Option A/B paths. JSON → TEMPLATE.md. Basketball removed.
- community/CONTRIBUTORS.md: Founding Calibrators table added. Basketball
  removed. JSON/path corrected. Sport priorities rewritten with $UFC/$PFL noted.
...
### Changed
- CONTRIBUTING-GAPS.md: Six-Month Test added to Library Rule. LAYER_GAP
  annotated with five SportMind layers.
...
### Changed
- CONTRIBUTING.md: MIND DIMENSIONS added as required section. Calibration record
  contribution section added. Aspirational infrastructure removed. Tier A list
  corrected: basketball, MotoGP, cricket removed. Co-maintainer section
  simplified. Founding Calibrators recognition added.
...
### Changed
- WHO-USES-THIS.md: Calibration counts: 126→137 and 100→137. MMA added ($UFC
  and $PFL confirmed). MCP server path added to decision tree and quick reference.
...
### Added
- FIRST-RECORD-GUIDE.md: Full rewrite. Option A (any LLM) + Option B (MCP
  server). Fan token validity rules, black logo signal, record type distinction.
  Founding Calibrators 10 slots. Historical WC2026 fixtures removed.
- community/calibration-data/TEMPLATE.md: New contributor calibration template.
  Full field set. No MIND DIMENSIONS section.
...
### Added
- fan-token/burn-to-glory-framework.md: Treasury size variation agent rule.
  Burn % is the comparable metric — not absolute amounts across tokens.
- macro/macro-regulatory-sportfi.md: GENIUS Act (enacted 2025) US payment
  stablecoin framework. Fan tokens NOT in scope. Effective 2027.
...
### Added
- fan-token/use-cases.md: Champion Call pre-match demand amplifier. Balance
  snapshot, proportional entry, holding requirement, winner-linked outcome.
  FM1/FM4/FM8 agent rules.
- macro/regulatory/uk-cryptoasset-regime.md: Five-PS architecture confirmed
  June 30 2026. MARC: insider trading now explicitly covered under UK law.
  Criminal liability from Q4 2027. Branch model for MiCA-authorised entities.
...
- macro/regulatory/global-regulatory-landscape.md: Japan FIEA Amendment enacted
  July 15 2026. Crypto reclassified as financial products. ETF pathway open.
  Tax 55%→20% flat rate January 2028. South Korea National Asset Basic Act
  announced July 14 2026. 2027 CBDC pilot.
...
- fan-token/defi-integration-intelligence.md: Omnichain bridge event behaviour
  pattern. Bidirectional movement during major tournament result windows.
  Three agent rules: FM7, no spot demand conflation, bidirectional.
...
### Added
- macro/fan-adoption-intelligence.md: Three-tier demand segmentation. US
  structural gap. APAC player-centric fandom FM1 modifier. All 14 Mind Dims.
- macro/partnerships.md: Three Chiliz wallet models. FanTokens.com two-layer
  architecture. KayenFi identified as Chiliz-native Fan Token DEX.

### Changed
- platform/ai-agent-economy.md: Two-layer intelligence architecture added.
- fan-token/defi-integration-intelligence.md: KayenFi named and encoded.
- fan-token/registry/complete-registry.md: $PEPPER entry updated with KayenFi.
- fan-token/use-cases.md: Livestream trading battle added as structural KO
  match engagement pattern. Dual-layer mechanic. FM4 and FM1 agent rules.
...
### Added
- macro/chz-tokenomics.md: Dragon8 dynamic supply model. Pre-2024 fixed supply
  (8.8B CHZ) marked as historical. No hard cap, 1.88% annual floor after ~14yr.
  EIP-1559 burn + CHZ buy-back burn. PoSA consensus. All 14 Mind Dimensions.
...
### Changed
- Website: Version stamps updated to v4.1.6.
- index.html: File count → 760. Calibration count → 134.
- scripts/sportmind_mcp.py: SPAIN, BELG, SFA, SAFA added to FAN_TOKEN_REGISTRY.
  ARG/POR burn counts updated. ITA non-qualification note. SNFT deprecated.
  CEX perp futures flag: PSG, BAR, ATM, JUV, CITY, AFC, GAL, ARG, POR.
...
Note: release requested as v3.68.0 — corrected to v3.96.3 (sequential
from v3.96.2).

index.html — Intelligence Agent suite card:
  View site URL corrected:
    Was: https://sportmind.github.io/sportmind-intelligence-agent/
    Now: https://sportmind.github.io/intelligence-agent/
  View site → button confirmed present (kit-btn-primary, green).
  All three suite cards now have identical button structure:
    View site → (kit-btn-primary) + GitHub → (kit-btn-ghost).
